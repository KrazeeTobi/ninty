$(function(){
	...
	$(function(){
		...
		var menulist = [
			'<li class=""><a href="/event/e3_2015/index.html"><img src="/event/e3_2015/common/img/nav_top.png" alt="トップ TOP" width="38" height="24"></a></li>',
			'<li class=""><a href="/event/e3_2015/lineup/index.html"><img src="/event/e3_2015/common/img/nav_lineup.png" alt="ラインナップ LINE UP" width="82" height="24"></a></li>',
			'<li class=""><a href="/event/e3_2015/event/index.html"><img src="/event/e3_2015/common/img/nav_event.png" alt="イベントレポート EVENT REPORT" width="112" height="24"></a></li>',
			'<li class=""><a href="/event/e3_2015/iwatter/index.html"><img src="/event/e3_2015/common/img/nav_iwatter.png" alt="イワッター IWATTER" width="68" height="24"></a></li>',
		];
		
		var uri_obj = Shared.util.parseURI();
		
		var menu_ul = $('.page_header .g_nav__inner');
		menu_ul.html("");
		$.each(menulist,function(i,x){
			i = i +1;
			var $li=$(x);
			var href_obj = Shared.util.parseURI($li.find('a').attr('href'));
			var dir_depth = href_obj.directory.split("/").length;
			if (dir_depth <= 4) {
				if(uri_obj.directory === href_obj.directory) {
					$li.addClass('cu');
				}
			} else {
				if(uri_obj.directory.indexOf(href_obj.directory) >= 0) {
					$li.addClass('cu');
				}
			}
			menu_ul.append($li);
	...
	$(function(){
		var fTop = $('.f_pagetop');
		$('#f_pagetop, .f_logo_ntd, .f_logo_e3-2015').hover(function(){
			fTop.addClass('f_pagetop_ov');
		},function(){
			fTop.removeClass('f_pagetop_ov');
		});
	});
	...
	if(Shared.ua.isiPhone || Shared.ua.is3DS || Shared.ua.isAndroidMobile){
		...
		// リダイレクト除外
		var not = (uri['file'] == 'exception.html');
		...
		if(!not){
			...
			for(var i=0; i<fragment.length; i++){
				if(fragment[i].indexOf('e3_2015') != -1){
	...
	$(function(){
		...
		var shareURL  = 'http://www.nintendo.co.jp/event/e3_2015/index.html';
...
*/
		...
		$('.g_sns .tw a').attr('href', twURL).bind('click', function(e){
			e.preventDefault();
			openSubWin(this.href, 'twitter', 550, 342);
		});

		$('.g_sns .fb a').attr('href', fbURL).bind('click', function(e){
			e.preventDefault();
			openSubWin(this.href, 'facebook', 600, 300);
		});


		setToolTip($('.g_sns .tw'), $('.tooltiptwitter'));
		setToolTip($('.g_sns .fb'), $('.tooltipfacebook'));
	});
	...
	ModalController.prototype = {
		...
		open : function(type, url, w, h, opacity, style, modalClass){
			...
			jQuery.when(loadIframe(iframe), showBackground()).done(function(iframe){
				...
				self.modalClose.animate({top:0}, 300, 'oX2');
		...
		__setSlide : function(iframe, style, w, h){
			...
			function cb(hasNext, hasPrev){
				if(hasNext){
					//btnNext.css({display:'block'}).animate({marginRight:-w/2-71}, 700, 'oX2');
					btnNext.show();
				}else{
					//btnNext.animate({marginRight:0}, 700, 'iX2');
					btnNext.hide();
				}
				if(hasPrev){
					//btnPrev.css({display:'block'}).animate({marginLeft:-w/2-71}, 700, 'oX2');
					btnPrev.show();
				}else{
					//btnPrev.animate({marginLeft:0}, 700, 'iX2');
					btnPrev.hide();
				}
			}
